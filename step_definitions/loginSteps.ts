...
When('ingresa las credenciales del usuario de prueba', async function (this: CustomWorld) {
    const loginPage = new LoginPage(this.page);
    const user = testData.validUser;
    await loginPage.login(user.email, user.password);
});

Then('el usuario debería iniciar sesión correctamente y ver su nombre en la barra de navegación', async function (this: CustomWorld) {
    const homePage = new HomePage(this.page);
    const user = testData.validUser;
...
When('ingresa las credenciales de un usuario inválido', async function (this: CustomWorld) {
    const loginPage = new LoginPage(this.page);
    const user = testData.invalidUser;
    await loginPage.login(user.email, user.password);
});

Then('el usuario debería ver el mensaje de error de credenciales incorrectas', async function (this: CustomWorld) {
    const loginPage = new LoginPage(this.page);
    const user = testData.invalidUser;
